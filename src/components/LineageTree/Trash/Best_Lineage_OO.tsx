...
// ── constants ──────────────────────────────────────────────────────────────
  const PANEL_GAP = 60;      // vertical gap between panels (SVG units)
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // const url = GID2
        //   ? `${API_BASE}/Exp/LineageExpressionDataDual?SM=${SM}&ExpColName=${GID}&ExpColName2=${GID2}`
        //   : `${API_BASE}/Exp/LineageExpressionData?SM=${SM}&ExpColName=${GID}`;
        console.log(url)
        const response = await fetch(url);
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const jsonData = await response.json();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setExpLoading(true);
        //const url = `${API_BASE}/scExp/ForLineageTree?DataSet=P009D01S&Gene=${ScGene}`;
        const url = ScGene2
        ...
        console.log(`Fetching single-cell expression data from: ${url}`);
        const response = await fetch(url);
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        const jsonData = await response.json();
        setColorData(jsonData);
  ...
  useEffect(() => {
    ...
    // Build a new transform: keep current translate, only update scale
    // centered on the SVG's center point so zoom feels natural
    const cx = width / 2;
    const cy = height / 2;
  ...
  useEffect(() => {
    ...
    cellLineages.forEach((cell, panelIdx) => {
      ...
      //console.log(expCol('Zapppaapppa', 100, ColorData))
      ...
      // x: push tree to the right so the leftmost branch starts near axis
      const x = - CellX1(LineageLongestA) + MarginLeft;
      // y: top margin within this panel; times from minStart
      const y = 50;
      // convert TP → panel-local Y: subtract minStart so panel always starts at y
      ...
      // vertical per-TP colored lines
      // vertical per-TP colored lines — MERGED into segments
      keys.forEach((key) => {
        const cx = x + CellX1(key);

        // TPs: merge consecutive same-color timepoints
        ...
        g_verti_exp.append("g").attr("class", `lines-${key}`)
          ...
          .on("click", function (_event, d) {
            ...
            // ── 2. reset all short labels to white ──
            rootG.selectAll<SVGTextElement, string>(".lines-clab text")
              .filter((k) => k.length < LabelShowLength)
              .attr("fill", labelColor);

            // ── 3. show + highlight clicked label ──
            rootG.selectAll<SVGTextElement, string>(".lines-clab text")
            ...
            // ── 4. draw branch-end triangles ──
            const triG = rootG.select<SVGGElement>("#triangle-indicators");
            triG.selectAll("*").remove(); // clear previous
            ...
            const tw = 6; // triangle half-width
            const th = 8; // triangle height

            // ▼ top triangle — points downward into the branch top
            triG.append("polygon")
              .attr("points", `${triX},${absY_top + th} ${triX - tw},${absY_top} ${triX + tw},${absY_top}`)
              .attr("fill", "red")
              .style("pointer-events", "none");

            // ▲ bottom triangle — points upward into the branch bottom
            triG.append("polygon")
            ...
            colorMode == 'expression' && setExpVal?.(ColorData[key]);
            //console.log(ColorData[key])
          });

        // NATPs: always DimGrey, so just one line per continuous run
      ...
      showlabellist && g_label.selectAll<SVGLineElement, string>("line")
        ...
        .attr("y1", (d) => {
            ...
            const later = a.End >= p.End ? a : p;   // pick the object with larger .End
      ...
      // cell name labels on branches — render ALL keys, hide long ones initially
    ...
    // resize: only track width; height is determined by content
  ...
  return (
    <div
      ...
      style={{ minHeight: 400, maxHeight: "500vh",
